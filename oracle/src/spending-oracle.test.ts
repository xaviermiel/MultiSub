...
describe("buildSubAccountState", () => {
  ...
  describe("SWAP operations", () => {
    ...
    it("should inherit timestamp when swapping acquired tokens", () => {
      ...
      const usdcQueue = state.acquiredQueues.get(
  ...
  describe("DEPOSIT operations", () => {
    it("should track deposit record with input and output tokens", () => {
      ...
      expect(
        state.acquiredBalances.get(TOKEN_ALINK.toLowerCase() as Address),
      ).toBe(100n);
    });
    ...
    it("should inherit timestamp from acquired input for deposit output", () => {
      ...
      const aLinkQueue = state.acquiredQueues.get(
        TOKEN_ALINK.toLowerCase() as Address,
      );
      ...
      expect(state.depositRecords[0].originalAcquisitionTimestamp).toBe(
        HOUR_AGO,
      );
    });
  });
  ...
  describe("WITHDRAW operations", () => {
    ...
    it("should consume deposit output tokens (aTokens) on withdraw", () => {
      ...
      const aLinkBalance = state.acquiredBalances.get(
        TOKEN_ALINK.toLowerCase() as Address,
      );
  ...
  describe("CLAIM operations", () => {
    ...
    it("should inherit oldest deposit timestamp for CLAIM", () => {
      ...
      const wethQueue = state.acquiredQueues.get(
...
describe("Complex real-world scenarios", () => {
  ...
  describe("Multiple deposits and partial withdrawals", () => {
    ...
    it("should handle withdrawal when output tokens have expired", () => {
      ...
      expect(
        state.acquiredBalances.get(TOKEN_LINK.toLowerCase() as Address) ?? 0n,
      ).toBe(0n);

      // aLINK should have 0 acquired balance (all expired)
      expect(
        state.acquiredBalances.get(TOKEN_ALINK.toLowerCase() as Address) ?? 0n,
      ).toBe(0n);
    });
    ...
    it("should handle multi-token LP deposits without double-counting output", () => {
      ...
      const usdcRecord = state.depositRecords.find(
        (r) => r.tokenIn.toLowerCase() === TOKEN_USDC.toLowerCase(),
      );
      const wethRecord = state.depositRecords.find(
        (r) => r.tokenIn.toLowerCase() === TOKEN_WETH.toLowerCase(),
      );
      ...
      const totalRemainingOutput = state.depositRecords.reduce(
        (sum, r) => sum + r.remainingOutputAmount,
        0n,
      );
      ...
      expect(
        state.acquiredBalances.get(TOKEN_LP.toLowerCase() as Address),
      ).toBe(500n);
    });
    ...
    it("should correctly consume LP tokens on withdrawal after multi-token deposit", () => {
      ...
      const lpBalance =
        state.acquiredBalances.get(TOKEN_LP.toLowerCase() as Address) ?? 0n;
    ...
    it("should handle single-input multi-output deposits with separate deposit records", () => {
      ...
      const recordA = state.depositRecords.find(
        (r) => r.tokenOut.toLowerCase() === TOKEN_RECEIPT_A.toLowerCase(),
      );
      const recordB = state.depositRecords.find(
        (r) => r.tokenOut.toLowerCase() === TOKEN_RECEIPT_B.toLowerCase(),
      );
      ...
      expect(
        state.acquiredBalances.get(TOKEN_RECEIPT_A.toLowerCase() as Address),
      ).toBe(50n);
      expect(
        state.acquiredBalances.get(TOKEN_RECEIPT_B.toLowerCase() as Address),
      ).toBe(30n);
    });
  ...
  describe("USD-weighted ratio calculation", () => {
    it("should use USD-weighted ratio for mixed multi-token swaps", () => {
      ...
      const priceCache: TokenPriceCache = new Map([
        [
          TOKEN_WETH.toLowerCase() as Address,
          { priceUSD: parseUnits("3000", 18), decimals: 18 },
        ],
        [
          TOKEN_USDC.toLowerCase() as Address,
          { priceUSD: parseUnits("1", 18), decimals: 6 },
        ],
      ]);
